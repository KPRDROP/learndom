...
EVENT_START_OFFSET_MINUTES = -240
EVENT_END_OFFSET_MINUTES = 240
...
def build_vlc_entry(key: str, entry: dict[str, Any]) -> str | None:
...
def build_tivimate_entry(key: str, entry: dict[str, Any]) -> str | None:
    source = entry.get("source")
    if not source:
        return None
    source = str(source).strip()
...
def write_playlists() -> None:
    ...
    sorted_urls = sorted(
        urls.items(),
        key=lambda item: (item[1].get("timestamp", 0), item[0].lower()),
    )
    ...
    with open("rdstrm_vlc.m3u8", "w", encoding="utf-8", newline="\n") as f:
        f.write(vlc_content)
    with open("rdstrm_tivimate.m3u8", "w", encoding="utf-8", newline="\n") as f:
        f.write(tivimate_content)
    log.info(f"Wrote rdstrm_vlc.m3u8 ({len(vlc_entries)} entries)")
    log.info(f"Wrote rdstrm_tivimate.m3u8 ({len(tivimate_entries)} entries)")


def _extract_m3u_from_embed(embed_url: str) -> str | None:
    if not embed_url:
        return None
    query = dict(parse_qsl(urlsplit(embed_url).query))
    m3u = query.get("url")
    if not m3u:
        return None
    return unquote(m3u)
...
async def pre_process(url: str, url_num: int) -> str | None:
    ...
    if not url:
        ...
        return None
    try:
        event_data = await network.request(url, url_num, log=log)
    except Exception as exc:
        log.error(f"URL {url_num}) Failed stream endpoint request: {exc}")
        return None
    if not event_data:
    ...
    for stream in streams:
        if not isinstance(stream, dict):
            continue
        source_name = str(stream.get("source") or "").strip().lower()
    ...
    for candidate in candidates:
        m3u = _extract_m3u_from_embed(candidate)
        if m3u:
            log.info(f"URL {url_num}) Captured M3U8")
            return m3u
        direct = normalize_url(candidate)
        if direct and direct.endswith(".m3u8"):
            log.info(f"URL {url_num}) Captured M3U8")
            return direct
...
async def load_api_data(now: Any) -> list[dict[str, Any]]:
    ...
    response = await network.request(EVENTS_API_URL, log=log)
    if not response:
        log.error(f"Unable to fetch events API: {EVENTS_API_URL}")
        return []
    try:
        data = response.json()
    except Exception as exc:
        log.error(f"Events API returned invalid JSON: {exc}")
        return []
    if not isinstance(data, list):
        log.error("Events API response is not a list")
        return []
    if data and isinstance(data[0], dict):
        log.info(f"API sample keys: {list(data[0].keys())}")
...
async def get_events(cached_keys: KeysView[str]) -> list[REEDEvent]:
    ...
    if not api_data:
        return events
    start_dt = now.delta(minutes=EVENT_START_OFFSET_MINUTES)
    end_dt = now.delta(minutes=EVENT_END_OFFSET_MINUTES)
    matched_count = 0
    ...
    for event in api_data:
        ...
        values = [
            event.get(x)
            for x in ("category", "title", "league_name", "date", "id")
        ]
        if not all(values):
            continue
        ...
        try:
            ...
            log.debug(f"Skipping event with invalid timestamp: {start_ts}")
            continue
        try:
            event_dt = Time.from_ts(event_ts)
        except Exception:
            log.debug(f"Skipping event with invalid event time: {start_ts}")
            continue
        ...
        logo = (
            urljoin(API_BASE_URL, str(poster).lstrip("/"))
            if poster
            else None
        )
        stream_link = urljoin(LINKS_BASE_URL, f"stream/{stream_id}")
        events.append(
            REEDEvent(
                sport=sport,
                name=name,
    ...
    log.info(
        ...
        f"[{EVENT_START_OFFSET_MINUTES}m, +{EVENT_END_OFFSET_MINUTES}m]"
    )
    return events
...
async def scrape(browser: Browser) -> None:
    ...
    async with network.event_context(browser) as context:
        for i, ev in enumerate(events, start=1):
            ...
            if not event_link:
                log.warning(f"URL {i}) Missing event link")
            else:
                source = await pre_process(event_link, i)
            key = f"[{ev.sport}] {ev.name} ({TAG})"
            tvg_id, league_logo = leagues.get_tvg_info(ev.sport, ev.name)
            ...
            if source:
                valid_count += 1
                urls[key] = entry
                log.info(f"URL {i}) Added stream for {key}")
            else:
                log.warning(f"URL {i}) No playable stream captured")
